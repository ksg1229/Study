              ...
              <header class="pv-head">
                ...
                <div class="pv-meta">
                  ...
                  <c:if test="${(not empty loginMemberId and loginMemberId == post.authorId) or (isAdmin == true)}">
                    ...
                    <form method="post" action="${pageContext.request.contextPath}/community/delete/${post.postId}"
                ...
                <div class="comment ${cmt.parentId != null ? 'reply' : ''}">
                  ...
                  <div class="comment-actions">
                    <button type="button" class="link"
                      onclick="setReplyParent('${cmt.commentId}','${cmt.authorId}')">답글</button>

                    <c:if test="${(not empty loginMemberId and loginMemberId == cmt.authorId) or (isAdmin == true)}">
                      <form method="post"
                        action="${pageContext.request.contextPath}/community/comments/${cmt.commentId}/delete">
                  ...
                  <div id="reply-box-${cmt.commentId}" class="reply-form" style="display:none">
                    <form method="post" action="${pageContext.request.contextPath}/community/comments"
                      class="comment-form">
                      <input type="hidden" name="postId" value="${post.postId}" />
                ...
                <nav class="pagination">
                  <c:choose>
                    <c:when test="${pageMeta.hasPrev}">
                      <a class="page"
                        href="${pageContext.request.contextPath}/community/view/${post.postId}?page=${pageMeta.page - 1}&size=${pageMeta.size}">이전</a>
                    </c:when>
                  ...
                  <c:forEach var="p" begin="${pageMeta.start}" end="${pageMeta.end}">
                    <c:choose>
                      ...
                      <c:otherwise><a class="page"
                          href="${pageContext.request.contextPath}/community/view/${post.postId}?page=${p}&size=${pageMeta.size}">${p}</a>
                      </c:otherwise>
                    </c:choose>
                  </c:forEach>
                  ...
                  <c:choose>
                    <c:when test="${pageMeta.hasNext}">
                      <a class="page"
                        href="${pageContext.request.contextPath}/community/view/${post.postId}?page=${pageMeta.page + 1}&size=${pageMeta.size}">다음</a>
                    </c:when>
