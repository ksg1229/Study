...
<div class="container">
  ...
  <section>
    <div class="card">
      <h2 style="margin:6px 0 10px;"><c:out value="${title != null ? title : '스터디 세션'}"/></h2>
      ...
      <div class="controls">
        ...
        <button id="end" class="hostOnly" style="margin-left:8px;color:#fff;background:#ef4444;border:0;padding:6px 10px;border-radius:6px">
  강의 종료
</button>
...
function bindControls(){
  ...
  chatFile.onchange = function(e){
    ...
    shrinkImage(f, 1280, 0.85, function(data){
      ...
      chatFile.value='';
      // (선택) 이미지 영구 저장이 필요하면 MESSAGE에 IMAGE 타입 저장 API 추가 필요
    });
  };
...
function sendChat(){
  ...
  // ★ DB 저장 + 목록 갱신
  if (window.$) $.post(API_BASE + '/messages', { msgType:'TEXT', content: v }).always(refreshMessages);
}
...
function saveMemoLocal(){
  ...
  g('memoInput').value='';
  // ★ DB 저장 + 목록 갱신
  if (window.$) $.post(API_BASE + '/messages', { msgType:'MEMO', content: v, atSec: at }).always(refreshMessages);
}
...
function redrawWB(){ hardClearCanvas();
for(var i=0;i<wb.history.length;i++) drawStroke(wb.history[i]); }
function clearWB(){ wb.history=[]; wb.redo=[];
hardClearCanvas(); }
function resizeWB(){
